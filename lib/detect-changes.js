...
function makePackageBuilt(lernaPackage, {log = npmlog} = {log: npmlog}) {
  return label => {
    log.verbose('makePackageBuilt', 'marking module built', {
      packagePath: lernaPackage.location,
      label
    })
...
function makePackageUnbuilt(lernaPackage, {log = npmlog} = {log: npmlog}) {
  return label => {
    log.verbose('makePackageUnbuilt', 'marking module unbuilt', {
      packagePath: lernaPackage.location,
      label
    })
...
function targetFileSentinelFile(lernaPackage, label = 'default') {
  const name = `.${lernaPackage.name}-${label}-sentinel`;
  const sanitizedName = sanitize(name, {replacement: '_'});
  return path.resolve(process.cwd(), '.lerna', sanitizedName);
}
  ...
  return entries
    ...
    .some(({relativePath, stats}) => {
      return stats.isDirectory()
        ? modifiedAfter(baseDir, relativePath, ignored, timeStamp)
        : stats.mtime.getTime() > timeStamp
    })
}
